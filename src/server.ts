...
try {
  ...
  interface GiveawayState {
    ...
    winner: string | null;
    winnerFound: boolean; // True if winner spoke in chat
  ...
  const giveawayStates = new Map<string, GiveawayState>();
  ...
  function formatGiveawayState(state: GiveawayState) {
    return {
      ...
      isAcceptingEntries: state.isAcceptingEntries,
  ...
  async function connectToChannel(channelName: string): Promise<void> {
    ...
    kickWS.on('ChatMessage', (message: any) => {
      ...
      // Giveaway Logic
      const giveaway = giveawayStates.get(channelName.toLowerCase());
      if (giveaway) {
        // 1. Add participant if active and typed !ticket 1
        if (giveaway.isActive && giveaway.isAcceptingEntries && message.content.toLowerCase().includes('!ticket 1')) {
          if (!giveaway.participants.has(username)) {
            giveaway.participants.add(username);
            io.to(`channel_${channelName.toLowerCase()}`).emit('giveaway_update', formatGiveawayState(giveaway));
            console.log(`[${channelName}] Participante añadido al sorteo: ${username}`);
          }
        }

        // 2. Check if winner spoke
        if (giveaway.winner && !giveaway.winnerFound && giveaway.winner === username && giveaway.timerSeconds > 0) {
          giveaway.winnerFound = true;
          if (giveaway.timer) {
            clearTimeout(giveaway.timer);
            giveaway.timer = null;
          }
          io.to(`channel_${channelName.toLowerCase()}`).emit('giveaway_update', formatGiveawayState(giveaway));
          console.log(`[${channelName}] ¡Ganador ${username} apareció!`);
        }
      }
    });
  ...
  io.on('connection', (socket) => {
    ...
    // --- Giveaway Logic ---
    socket.on('start_giveaway', (data: { channel: string }) => {
      const channel = data.channel.toLowerCase();
      let state = giveawayStates.get(channel);
      if (!state) {
        state = {
          isActive: true,
        ...
        giveawayStates.set(channel, state);
      } else {
        state.isActive = true;
      ...
      io.to(`channel_${channel}`).emit('giveaway_update', formatGiveawayState(state));
      console.log(`[${channel}] Sorteo iniciado`);
    });

    socket.on('stop_entries', (data: { channel: string }) => {
      const channel = data.channel.toLowerCase();
      const state = giveawayStates.get(channel);
      if (state) {
        state.isAcceptingEntries = false;
        io.to(`channel_${channel}`).emit('giveaway_update', formatGiveawayState(state));
        console.log(`[${channel}] Inscripciones cerradas`);
      }
    });

    socket.on('end_giveaway', (data: { channel: string }) => {
      const channel = data.channel.toLowerCase();
      const state = giveawayStates.get(channel);
      if (state) {
        ...
        state.isAcceptingEntries = false;
        io.to(`channel_${channel}`).emit('giveaway_update', formatGiveawayState(state));
        console.log(`[${channel}] Sorteo finalizado (escucha detenida)`);
      }
    });

    socket.on('pick_winner', (data: { channel: string }) => {
      const channel = data.channel.toLowerCase();
      const state = giveawayStates.get(channel);
      if (state && state.participants.size > 0) {
        ...
        io.to(`channel_${channel}`).emit('giveaway_update', formatGiveawayState(state));
        console.log(`[${channel}] Ganador elegido: ${winner}`);
      }
    });
  });
